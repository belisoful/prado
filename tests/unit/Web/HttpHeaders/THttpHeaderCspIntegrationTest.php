 ...
 *   ↔ TCspReporterService (wired via ReporterMode / ReporterServiceId).
 *
 * Tests cover:
 * - report-to ↔ Reporting-Endpoints cross-validation (finalizeHeader warning)
 * - ReporterMode=true  injects Reporting-Endpoints + report-to directive
 * - ReporterMode='Auto' additionally converts enforcing CSP to report-only
 ...
 * - Custom (literal) ReporterServiceId is used in the endpoint URL
...
use Prado\Web\Services\TCspReporterService;
...
class THttpHeaderCspIntegrationTest extends PHPUnit\Framework\TestCase
{
	...
	/** Inject a TCspReporterService entry into the app's service registry and
	 * return a callable that restores the original state. */
	private function injectService(string $id = TCspReporterService::SERVICE_ID): callable
	{
		...
		$ref->setValue(self::$app, array_merge($before, [
			$id => [TCspReporterService::class, [], null],
		]));
	...
	// -----------------------------------------------------------------------
	// ReporterMode = true — automatic injection
	// -----------------------------------------------------------------------

	public function testReporterModeTrueInjectsReportingEndpointsHeader()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
			$manager->setReporterMode(true);
			$manager->publicFinalizeReporterService();

			$reHeaders = $manager->getHeadersByClass(THttpHeaderReportingEndpoints::class);
			self::assertNotEmpty($reHeaders, 'A Reporting-Endpoints header must be injected when ReporterMode=true');
		} finally {
			$restore();
		}
	}

	public function testReporterModeTrueInjectsReportToIntoCsp()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
	...
	public function testReporterModeTrueReportToNameMatchesReportingEndpointsName()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
	...
	public function testReporterModeTrueDoesNotConvertCspToReportOnly()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
			...
			self::assertFalse($csp->getReportOnly(),
				'ReporterMode=true must NOT convert an enforcing CSP to report-only');
		} finally {
			$restore();
		}
	}

	public function testReporterModeTrueEmitsCorrectHeaderStrings()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
			...
			$hasCsp = false;
			$hasRe = false;
			$endpointName = $manager->getReporterEndpointName();
			foreach ($manager->sentHeaders as $h) {
				if (str_starts_with($h, 'Content-Security-Policy:')) {
					$hasCsp = true;
	...
	// -----------------------------------------------------------------------
	// ReporterMode = 'Auto' — additionally converts CSP to report-only
	// -----------------------------------------------------------------------

	public function testReporterModeAutoConvertsEnforcingCspToReportOnly()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
			$manager->setReporterMode('Auto');
			$manager->publicFinalizeReporterService();

			/** @var THttpHeaderCsp $csp */
			$csp = $manager->getHeadersByClass(THttpHeaderCsp::class)[0];
			self::assertTrue($csp->getReportOnly(),
				'ReporterMode=Auto must convert an enforcing CSP to report-only');
		} finally {
			$restore();
		}
	}

	public function testReporterModeAutoLeavesAlreadyReportOnlyCspUnchanged()
	{
		$restore = $this->injectService();
		try {
			// CSP is already report-only — Auto must not change it (it stays report-only).
			...
			$csp = $manager->getHeadersByClass(THttpHeaderCsp::class)[0];
			self::assertTrue($csp->getReportOnly(),
				'An already-report-only CSP must remain report-only after Auto mode');
		} finally {
			$restore();
		}
	}

	public function testReporterModeAutoEmitsReportOnlyHeaderName()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
			...
			foreach ($manager->sentHeaders as $h) {
				...
				if (str_starts_with($h, 'Content-Security-Policy:')) {
					self::fail('Enforcing Content-Security-Policy header must not be emitted in Auto mode');
				}
			}
			self::assertTrue($hasReportOnly, 'Content-Security-Policy-Report-Only header must be emitted in Auto mode');
		} finally {
			$restore();
		}
	}

	public function testReporterModeAutoStillInjectsReportingEndpointsAndReportTo()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
	...
	public function testNoDoubleInjectionOfEndpointWhenAlreadyDeclared()
	{
		$restore = $this->injectService();
		try {
			$endpointName = 'prado-csp-reporter'; // default ReporterEndpointName
	...
	public function testReporterModeTrueInjectsReportToIntoAllCspHeaders()
	{
		$restore = $this->injectService();
		try {
			$manager = new TTestableHttpHeadersManager();
	...
	public function testCustomReporterEndpointNameAppearsInHeaders()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
			$manager->setReporterMode(true);
			$manager->setReporterEndpointName('my-custom-endpoint');
			$manager->publicFinalizeReporterService();
	...
	public function testCustomReporterEndpointNameAppearsInEmittedHeaderStrings()
	{
		$restore = $this->injectService();
		try {
			$manager = $this->makeManagerWithCsp();
	...
	// -----------------------------------------------------------------------
	// Custom ReporterServiceId (literal)
	// -----------------------------------------------------------------------

	public function testLiteralReporterServiceIdAppearsInEndpointUrl()
	{
		$manager = $this->makeManagerWithCsp();
		$manager->setReporterMode(true);
		$manager->setReporterServiceId('my-csp-service');
		$manager->publicFinalizeReporterService();

		/** @var THttpHeaderReportingEndpoints $re */
		$re = $manager->getHeadersByClass(THttpHeaderReportingEndpoints::class)[0];
		self::assertStringContainsString('my-csp-service', $re->getHeaderValue(),
			'The literal ReporterServiceId must appear in the endpoint URL');
	}

	// -----------------------------------------------------------------------
	// CSP initComplete — sandbox directive is stripped in report-only mode
	// -----------------------------------------------------------------------
	...
	public function testSandboxDirectiveIsStrippedWhenCspConvertedToReportOnly()
	{
		...
		try {
			...
			// initComplete() is NOT called during loadHeaders() — only init() is.
			// So sandbox is still present after loading. Auto mode converts the
			// enforcing CSP to report-only and then calls initComplete() explicitly,
			// which strips the sandbox directive.
			$manager->setReporterMode('Auto');
			$manager->publicFinalizeReporterService();
	...
	public function testFullPipelineEmitsAllRequiredHeaders()
	{
		...
		try {
			$manager = new TTestableHttpHeadersManager();
			$manager->setReporterMode(true);
			$manager->setReporterEndpointName('prado-csp');
			$manager->publicLoadHeaders([
				'headers' => [
					[
						'class'    => THttpHeaderCsp::class,
	...
	// -----------------------------------------------------------------------
	// TCspReporterService auto-registration via ensureReporterServiceRegistered
	// -----------------------------------------------------------------------

	public function testReporterModeTrueAutoRegistersServiceInAppRegistry()
	{
		$m = new TTestableHttpHeadersManager();
		$m->setReporterMode(true);
		$m->setId('headers');

		$ref = new ReflectionProperty(TApplication::class, '_services');
		$ref->setAccessible(true);
		$before = $ref->getValue(self::$app);
		$without = array_filter($before, fn ($s) => ($s[0] ?? null) !== TCspReporterService::class);
		$ref->setValue(self::$app, $without);

		try {
			$m->ensureReporterServiceRegistered(self::$app, null);
			$services = $ref->getValue(self::$app);
			$byClass = array_filter($services, fn ($s) => ($s[0] ?? null) === TCspReporterService::class);
			self::assertNotEmpty($byClass,
				'ReporterMode=true must register TCspReporterService in the app registry');
		} finally {
			$ref->setValue(self::$app, $before);
		}
	}

	public function testReporterModeAutoAutoRegistersServiceInAppRegistry()
	{
		$m = new TTestableHttpHeadersManager();
		$m->setReporterMode('Auto');
		$m->setId('headers');
		...
		try {
			...
			$byClass = array_filter($services, fn ($s) => ($s[0] ?? null) === TCspReporterService::class);
			self::assertNotEmpty($byClass,
				'ReporterMode=Auto must register TCspReporterService in the app registry');
		} finally {
			$ref->setValue(self::$app, $before);
		}
	}
	...
	public function testAutoRegisteredServiceInitPropertiesContainAutoRegisteredTrue()
	{
		...
		try {
			...
			$entry = $services[TCspReporterService::SERVICE_ID] ?? null;
	...
	public function testReporterUrlInReportingEndpointsContainsServiceId()
	{
		...
		try {
			...
			self::assertStringContainsString(TCspReporterService::SERVICE_ID, $reporterUrl,
	...
	public function testViolationLegacyFormatIsProcessedByService()
	{
		...
		try {
			...
			self::assertSame('https://evil.example/x.js', $capturedParam->getBlockedUri());
			self::assertSame('script-src-elem', $capturedParam->getEffectiveDirective());
			self::assertSame('https://example.com/page', $capturedParam->getDocumentUri());
	...
	public function testViolationModernFormatIsProcessedByService()
	{
		...
		try {
			...
			self::assertSame(204, $svc->getStatusCode());
			self::assertInstanceOf(TCspViolationParameter::class, $capturedParam);
			self::assertSame('https://evil.example/track.js', $capturedParam->getBlockedUri());
			self::assertSame('script-src-elem', $capturedParam->getEffectiveDirective());
			self::assertSame('https://example.com/page', $capturedParam->getDocumentUri());
	...
	// -----------------------------------------------------------------------
	// Helper: anonymous TCspReporterService subclass for violation tests
	// -----------------------------------------------------------------------

	/**
	 * Returns a minimal anonymous subclass of TCspReporterService that
	 * intercepts readBody(), getRequest(), and getResponse() so no live HTTP
	 * stack or TApplication wiring is required.
	 */
	private function makeTestableReporterService(string $requestMethod, string|false $body): TCspReporterService
	{
		return new class ($requestMethod, $body) extends TCspReporterService {
